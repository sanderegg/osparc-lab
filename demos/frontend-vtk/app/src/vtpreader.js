...
addDataSetButton.addEventListener('click', () => {
  ...
  rootControllerContainer.style.display = 'flex';
});
...
function createPipeline(fileName, fileContents) {
  ...
  const sum = dataRange.reduce((previous, current) => current += previous);
  let average = sum / dataRange.length;

  const dataRangeSelector = document.createElement("input");
  dataRangeSelector.setAttribute('class', selectorClass);
  dataRangeSelector.setAttribute('type', 'range');
  dataRangeSelector.setAttribute('value', average);
  dataRangeSelector.setAttribute('min', dataRange[0] );
  dataRangeSelector.setAttribute('max', dataRange[1] );
  controlContainer.appendChild(dataRangeSelector);
